...
app.set( 'views', 'lib/poc/oauth/views' );
...
app.get( '/google_login', function ( req, res ) {
    ...
    var oa = new OAuth(
        ...
        "anonymous",
        "1.0",
        "http://localhost:3000/google_cb" + (
            req.param( 'action' ) &&
                req.param( 'action' ) != "" ? "?action=" + querystring.escape( req.param( 'action' ) ) : ""
            ),
        "HMAC-SHA1"
    );
...
app.get( '/google_cb', function ( req, res ) {
    ...
    var oa = new OAuth(
        req.session.oa._requestUrl, req.session.oa._accessUrl, req.session.oa._consumerKey,
        req.session.oa._consumerSecret, req.session.oa._version, req.session.oa._authorize_callback,
    ...
    oa.getOAuthAccessToken(
        req.session.oauth_token, req.session.oauth_token_secret,
...
app.get( '/google_calendars', require_google_login,
    function ( req, res ) {
        var oa = new OAuth(
            req.session.oa._requestUrl,
            req.session.oa._accessUrl,
        ...
        oa.getProtectedResource(
            ...
            function ( error, data, response ) {
                ...
                res.render( './views/google_calendars.ejs', { locals: { feed: feed } } );
            }
        );
